...
def limo_update(grad, momentum_buffer, momentum, momentum_2, ns_steps, nesterov, rms_scale, use_scale, eps):
    """
    LiMO update function with double momentum and sign operation.
    """
...
class Fsdp1dWork:
    ...
    def start(self):
        # Apply double momentum for LiMO
    ...
    def finish(self):
        ...
        if rank == dest_rank:
            ...
            # Apply LiMO update: NS orthogonalization + sign
            g_full_block = fast_newtonschulz(g_full_block, steps=self.group["ns_steps"])
            g_full_block = torch.sign(g_full_block)  # LiMO sign operation
...
class LiMo(torch.optim.Optimizer):
        ...
        for group in param_groups:
            ...
            if group["use_limo"]:
                ...
                if self.is_deepspeed_enabled and rms_scale:
                    logger.warning("rms_scale for LiMO is not supported in deepspeed, setting rms_scale to False")
                    rms_scale = False
                group["rms_scale"] = rms_scale
                nesterov = group.get("nesterov", True)
                if self.is_deepspeed_enabled and not nesterov:
                    logger.warning("disabled nesterov for LiMO is not supported in deepspeed, setting nesterov to True")
                    nesterov = True
                ...
                group["weight_decay"] = group.get("weight_decay", 0)
                assert set(group.keys()) == set(["params", "lr", "betas", "eps", "weight_decay", "use_limo"])
    ...
    def step_deepspeed(self, closure=None):
        ...
        for group in self.param_groups:
            if group["use_limo"]:
                # for deepspeed, if we set the param.use_limo as True, the update is done in the deepspeed's framework.
